...
class SlitDevice(Device):
    ...
    top = FormattedComponent(
        EpicsMotor, "{prefix}{_motorsDict[top]}", labels=("motor",)
    )

    bot = FormattedComponent(
        EpicsMotor, "{prefix}{_motorsDict[bot]}", labels=("motor",)
    )

    out = FormattedComponent(
        EpicsMotor, "{prefix}{_motorsDict[out]}", labels=("motor",)
    )

    inb = FormattedComponent(
        EpicsMotor, "{prefix}{_motorsDict[inb]}", labels=("motor",)
    )
