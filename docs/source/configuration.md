...
| Functional (any string) | Used for `find_loadable_devices(label=...)` |

Functional labels currently in use across `devices.yml` include
`"detector"`, `"motor"`, `"slit"`, `"phase retarder"`, `"track_energy"`,
`"energy device"`, `"feedback"`, `"camera"`, `"flag"`, `"kb"`,
`"high pressure"`, `"qxscan"`, `"temperature"`, `"preamp"`,
`"filter"`, `"shutter"`, `"xbpm"`, `"status"`, `"source"`, `"table"`,
`"magnet"`, `"mirror"`. Any string is allowed — these are the
conventional ones already in use.
