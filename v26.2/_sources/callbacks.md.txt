...
# → creates a file like: 04_09_my_experiment.dat
...
# After a scan — each is a dict keyed by the detector signal name:
peaks.com     # x at center-of-mass (centroid)
peaks.cen     # x at FWHM midpoint
peaks.max     # x at maximum y
peaks.min     # x at minimum y
peaks.fwhm    # full-width at half-max
```

`peaks.com` and `peaks.cen` are **not** the same — `com` is the
moment-based centroid, `cen` is the midpoint of the FWHM crossings.
For an asymmetric peak they differ.

For an automation-friendly API that also moves the positioner, prefer
the [peak-finding plans](plans.md#peak-finding-plans) (`RE(cen())`,
`RE(maxi())`, …) — they read the same scan data but are usable inside
plans and macros without going through the BEC singleton.
